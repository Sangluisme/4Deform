...
class BehaveShapeGenerate:
    def __init__(self, data_root, save_dir, point_num=20000):
        
        assert os.path.isdir(data_root), f'Invaid data root:{data_root}.'
        
        self.data_root = data_root
        self.save_root = os.path.join(save_dir, 'behave')
        self.point_num=point_num

        
        utils.mkdir_ifnotexists(self.save_root)
        
        self.subfolders = [f.name for f in os.scandir(self.data_root) if f.is_dir()]
        self.subfolders = sort_list(self.subfolders)
        
        self.sequence_num = len(self.subfolders)
        
    
    def setup(self, subfolder_index):
        # self.save_dir = os.path.join(self.save_root, self.subfolders[subfolder_index])
        
        name = self.subfolders[subfolder_index].split('_')[2:]
        self.scene_name = self.subfolders[subfolder_index].split('_')[2]
        
        self.save_dir = os.path.join(self.save_root, '_'.join(name))
        utils.mkdir_ifnotexists(self.save_dir)
        
        # self.smpl_save_folder = os.path.join(self.save_dir, 'register')
        ...
        self.ptc_save_folder = os.path.join(self.save_dir, 'ptc')
        utils.mkdir_ifnotexists(self.ptc_save_folder)
        ...
        self.person_folder = os.path.join(self.save_dir, 'person')
        utils.mkdir_ifnotexists(self.person_folder)
        
        self.scence_folder = os.path.join(self.save_dir, 'scene')
        utils.mkdir_ifnotexists(self.scence_folder)
        
        self.folder = os.path.join(self.data_root, self.subfolders[subfolder_index])
        mesh_folders = [f.name for f in os.scandir(self.folder) if f.is_dir()]
        self.mesh_folders = sort_list(mesh_folders)
        
                    
    def __len__(self):
        return self.sequence_num
        
    def get_mesh_from_sequence(self, prefix=None):
        
        for f in self.mesh_folders:
            person_ply = os.path.join(self.folder, f, 'person', 'fit02', 'person_fit.ply')
            scene_ply = os.path.join(self.folder, f, self.scene_name, 'fit01', self.scene_name + '_fit.ply')
            
            new_person_file = os.path.join(self.person_folder, f.split('.')[0] + ".ply")
            new_scene_file = os.path.join(self.scence_folder, f.split('.')[0] + ".ply")
            
            try:
                shutil.copy2(person_ply, new_person_file)
            except:
                pass
            
            try:
                shutil.copy2(scene_ply, new_scene_file)
            except:
                scene_ply = os.path.join(self.folder, f, prefix, 'fit01', prefix + '_fit.ply')
                # new_scene_file = os.path.join(scence_folder, prefix + ".ply")
                shutil.copy2(scene_ply, new_scene_file)

        print('copied SMPL and object meshes.')
    
    def create_external_pointcloud(self, prefix=""):
        
        for f in self.mesh_folders:
            person_ply = os.path.join(self.folder, f, 'person',  'person.ply')
            scene_ply = os.path.join(self.folder, f, self.scene_name,  self.scene_name + '.ply')
            
            person_ptc = trimesh.load(person_ply)
            try:
                scene_ptc = trimesh.load(scene_ply)
            except:
                try:
                    scene_ply = os.path.join(self.folder, f, prefix,  prefix + '.ply')
                    scene_ptc = trimesh.load(scene_ply)
                except:
                    pass
            
            points =jnp.concatenate([person_ptc.vertices, scene_ptc.vertices])
            colors = jnp.concatenate([person_ptc.colors, scene_ptc.colors])
            ...
            new_person_file = os.path.join(self.ptc_save_folder, f.split('.')[0] + ".ply")
            mesh_utils.save_pointcloud(points=points, color=colors, filename=new_person_file)

        print('Created external kinect point cloud.')
    
    def create_pointcloud(self):
        
        mesh_paths = [f for f in os.listdir(self.mesh_save_folder) if f.endswith('.ply')]
        mesh_paths = sort_list(mesh_paths)
        for mesh_name in mesh_paths:
            prefix = mesh_name.split('.')[0]
            mesh_files =os.path.join(self.mesh_save_folder, mesh_name)
            ...
            filename = os.path.join(self.ptc_save_folder, prefix + '.ply')
            mesh_utils.save_pointcloud(points, filename)
        print('created training point cloud and flax point cloud.')
    
    
    def create_training_pairs(self, step_length=1):
        ...
        for i in indices:
            ...
            filename = os.path.join(self.train_folder, flax_paths[i].split('.')[-2] + '-' +flax_paths[i+step_length].split('.')[-2])
    ...
    def create_whole_mesh(self):
        
        person_files = [f for f in os.listdir(self.person_folder)]
        person_files = sort_list(person_files)
        scene_files = [f for f in os.listdir(self.scence_folder)]
        scene_files = sort_list(scene_files)
        
        for index in range(len(person_files)):
            person = trimesh.load(os.path.join(self.person_folder, person_files[index]))
            scene = trimesh.load(os.path.join(self.scence_folder, scene_files[index]))
            index_num = len(person.vertices)
            person_face = person.faces
            scene_face = scene.faces + index_num
            faces = jnp.concatenate([person_face, scene_face])
            vertices = jnp.concatenate([person.vertices, scene.vertices])
            ...
            files = os.path.join(self.mesh_save_folder, person_files[index])
            mesh.export(files)
        
        print('created whole meshes.')
    
    def process(self):
        
        for subfolder_index in range(self.sequence_num):
            
            print('process subfolder {0}....\n'.format(self.subfolders[subfolder_index]))
            
            self.setup(subfolder_index=subfolder_index)
            
            self.get_mesh_from_sequence()
            self.create_whole_mesh()
            self.create_pointcloud()
            self.create_external_pointcloud()
...
class Dress4DShapeGenerate:
    def __init__(self, data_root, save_dir, point_num=20000):
        ...
        self.data_root = data_root
        self.save_root = os.path.join(save_dir,'dress4d')
        self.point_num=point_num
    ...
    def setup(self, subfolder_index):
        ...
        self.smpl_folder = os.path.join(self.data_root, self.subfolders[subfolder_index], 'SMPL')
        self.smpl_paths = [f for f in os.listdir(self.smpl_folder) if f.endswith('.ply')]
        self.smpl_paths = sort_list(self.smpl_paths)
        
        self.mesh_folder = os.path.join(self.data_root, self.subfolders[subfolder_index], 'Meshes_pkl')
    ...
    def create_pointcloud(self):
        ...
        for mesh_name in mesh_paths:
            ...
            #external mesh
            mesh_file =os.path.join(self.mesh_save_folder, mesh_name)
            points = trimesh.load(mesh_file).vertices
            colors =  trimesh.load(mesh_file).visual.vertex_colors

            filename = os.path.join(self.ptc_folder, prefix + '.ply')
            mesh_utils.save_pointcloud(points, filename, color=colors)
        print('done.')
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--seq_num',type=int, default=0, help='only for 4d-dress and behave')
    ...
    if args.data_type =='matching':
        ...
        dataset.process()
    elif args.data_type == 'dress4d':
        dataset = Dress4DShapeGenerate(data_root=args.data_root, save_dir=args.save_dir)
        dataset.process()
    elif args.data_type == 'behave':
        dataset = BehaveShapeGenerate(data_root=args.data_root, save_dir=args.save_dir)
        dataset.process()
    else:
