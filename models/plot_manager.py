...
class PlotManager:
    ...
    def extract_mesh(self, f, time_step, index=None, features=None, connected=True) -> trimesh.Trimesh:
        ...
        for inputs in voxel_slice:
            ...
            if features is not None:
                condition = jnp.tile(features, (inputs.shape[0],1))
                sdf = f(inputs, t=t, condition=condition)
            else:
                sdf = f(inputs, t=t)
            outputs_numpy.append(sdf)
    ...
    def visualize_velocity(self, input_x, f, time_step=None, index=None, latent_vec=None, feature=None):
        ...
        for p in points_slice:
            ...
            if latent_vec is not None:
                velocity = f(p, t=t,  latent_vec=latent_vec)
            elif feature is not None:
                condition = jnp.tile(feature, (p.shape[0],1))
                velocity = f(p, t=t, condition=condition)
            elif index is not None:
                velocity = f(p, t=t, index=index, latent_vec=None)
            else:
    ...
    def export_mesh(self, mesh, output_file=" "):
        filename = os.path.join(self.directory, self.prefix + output_file + '.ply')
        mesh.export(filename, 'ply')
    
    def save_mesh(self, verts, faces, color=None, filename=""):
        filename = os.path.join(self.directory, self.prefix +filename + '.ply')
    ...
    def save_pointcloud(self, points, normals, color=None, output_file=""):
        filename = os.path.join(self.directory, self.prefix +output_file + '.ply')
        if normals is not None:
            save_ptc_with_normal(points, normals, color, filename)
        else:
            if color is not None:
...
def save_ptc_with_normal(points, normals, color=None, output_file=""):
    num_verts = points.shape[0]
    if color is not None:
        dtypes = [("x", "f4"), ("y", "f4"), ("z", "f4"), ("nx", "f4"), ("ny", "f4"), ("nz", "f4"), ("red","u1"), ("green","u1"),("blue","u1")]
        verts = np.hstack((points, normals, color))
    else:
